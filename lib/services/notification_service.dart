...
class NotificationService {
  ...
  static Future<bool> sendFCMNotification({
    ...
    final payload = {
      'message': {
        ...
        'data':
            data ??
            {"click_action": "FLUTTER_NOTIFICATION_CLICK", "type": "general"},
      },
    };
  ...
  static Future<void> initialize() async {
    ...
    if (!kIsWeb && defaultTargetPlatform == TargetPlatform.iOS) {
      ...
      try {
        ...
        try {
          ...
          if (isSimulator && kDebugMode) {
            print(
              '📱 Running on iOS Simulator - APNS token functionality is limited',
            );
            print(
              '💡 APNS tokens are not available in iOS Simulator - this is expected',
            );
        ...
        int retryCount = 0;
        const maxRetries =
            5; // Reduced retries since simulators are handled above
        ...
        while (apnsToken == null && retryCount < maxRetries) {
          ...
          if (kDebugMode) {
            print(
              '⏳ APNS token not available yet, waiting... (attempt $retryCount/$maxRetries)',
            );
          }
        ...
        if (apnsToken == null) {
          if (kDebugMode) {
            print(
              '⚠️ APNS token still not available after $maxRetries attempts on physical device',
            );
            print(
              '🔍 This indicates APNS configuration issues - check certificates and provisioning',
            );
          }
        }
      } catch (e) {
        if (kDebugMode) {
          ...
          print(
            'This is normal on iOS simulator or when APNS is not configured',
          );
        }
      }
      ...
      try {
        ...
        if (settings.authorizationStatus == AuthorizationStatus.authorized) {
          if (kDebugMode) {
            print('✅ Web notifications authorized');
          }
        } else if (settings.authorizationStatus == AuthorizationStatus.denied) {
          if (kDebugMode) {
            print('❌ Web notifications denied by user');
          }
        } else {
          if (kDebugMode) {
            print('⚠️ Web notifications not determined');
          }
        }
      } catch (e) {
        if (kDebugMode) {
    ...
    if (!kIsWeb && defaultTargetPlatform == TargetPlatform.iOS) {
      ...
      // Web FCM token handling - requires user to have granted permission
      try {
        // For web, we need to get the token after permission is granted
        // The getToken() might need a VAPID key for production
        String? token = await _firebaseMessaging.getToken();
        if (token != null) {
          if (kDebugMode) print('🔑 Web FCM token obtained: $token');
          await _saveTokenToDatabase(token);
        } else {
          if (kDebugMode) {
            print(
              '⚠️ Web FCM token is null - permission may not be granted or VAPID key may be missing',
            );
            print('ℹ️  For production web notifications, ensure:');
            print('   1. User has granted notification permission');
            print('   2. VAPID key is configured in Firebase Console');
            print('   3. firebase-messaging-sw.js is properly registered');
          }
        }
      } catch (e) {
        if (kDebugMode) {
          print('⚠️ Error getting web FCM token: $e');
        }
      }
    } else {
      // Android FCM token handling
      String? token = await _firebaseMessaging.getToken();
      if (token != null) {
  ...
  /// Request notification permission for web explicitly
  /// Call this method after user interaction to ensure proper permission prompt
  static Future<bool> requestWebNotificationPermission() async {
    if (!kIsWeb) {
      if (kDebugMode) {
        print('⚠️ requestWebNotificationPermission called on non-web platform');
      }
      return false;
    }
    ...
    try {
      ...
      if (settings.authorizationStatus == AuthorizationStatus.authorized) {
        ...
        if (token != null) {
          if (kDebugMode) {
            print('🔑 Web FCM token obtained: $token');
          }
          await _saveTokenToDatabase(token);
        } else {
          if (kDebugMode) {
            print('⚠️ Web FCM token is null despite authorization');
          }
        }
      ...
      if (kDebugMode) {
        print('⚠️ Error requesting web notification permission: $e');
      }
      return false;
    }
  }
  ...
  static Future<void> _saveTokenToDatabase(String token) async {
    ...
    if (user != null) {
      ...
      if (kDebugMode) {
        print(
          '⚠️ User not found in any collection (teacher/student/admin): ${user.uid}',
        );
      }
    }
  }
  ...
  static Future<void> sendChatNotification({
    ...
    try {
      ...
      if (kDebugMode) {
        print(
          '✅ Chat notification sent to user ($receiverId) with background support',
        );
      }
    } catch (e) {
      if (kDebugMode) {
  ...
  static Future<void> sendClassScheduledNotification({
    ...
    try {
      ...
      if (kDebugMode) {
        print(
          '✅ Class scheduled notification sent successfully to $receiverId',
        );
      }
    } catch (e) {
      if (kDebugMode) {
  ...
  static Future<void> sendClassReminderNotification({
    ...
    try {
      ...
      final timeDescription =
          isMissed ? 'was scheduled to start' : 'starts in 5 minutes';
      final title =
          isMissed ? 'Missed Class Reminder' : '🔔 Class Starting Soon!';
      final body =
          isForTeacher
      ...
      if (kDebugMode) {
        print(
          '✅ Class reminder notification sent to user ($receiverId) with background support',
        );
      }
    } catch (e) {
      if (kDebugMode) {
  ...
  static Future<void> _showClassReminderLocalNotification(
    String title,
    String body,
  ) async {
    try {
  ...
  static Future<void> _saveNotificationToCorrectCollection(
    String userId,
    ...
    print('   userId: $userId');
    print(
      '   notification: ${notificationData['title']} - ${notificationData['body']}',
    );
    ...
    try {
      ...
      if (teacherDoc.exists) {
        ...
        print(
          '   Teacher: ${teacherData['fullName']} (${teacherData['email']})',
        );
    ...
    if (!saved) {
      try {
        ...
        if (studentDoc.exists) {
          ...
          print(
            '   Student: ${studentData['fullName']} (${studentData['email']})',
          );
    ...
    if (!saved) {
      try {
        ...
        if (kDebugMode) {
          print(
            '✅ Notification saved to fallback users collection for: $userId',
          );
        }
      } catch (e) {
        if (kDebugMode) {
          print(
            '❌ Critical error: Could not save notification anywhere for user $userId: $e',
          );
    ...
    try {
      ...
      try {
        ...
        if (teacherDoc.exists) {
          ...
          print(
            '   Found ${fcmTokens.length} FCM tokens in teachers collection',
          );
        }
      } catch (e) {
      ...
      if (fcmTokens.isEmpty) {
        try {
          ...
          if (studentDoc.exists) {
            ...
            print(
              '   Found ${fcmTokens.length} FCM tokens in students collection',
            );
          }
        } catch (e) {
      ...
      if (fcmTokens.isEmpty) {
        try {
          ...
          if (adminDoc.exists) {
            ...
            print(
              '   Found ${fcmTokens.length} FCM tokens in admin collection',
            );
          }
        } catch (e) {
          // Final fallback
      ...
      if (fcmTokens.isNotEmpty) {
        ...
        print(
          '📊 FCM Summary: $successCount/$totalTokens successful for user: $userId ($foundInCollection)',
        );
      } else {
        if (kDebugMode) {
          print(
            '⚠️ No FCM tokens found for user: $userId (check network connectivity)',
          );
        }
      }
    } catch (e) {
      if (kDebugMode) {
        ...
        print(
          '🔍 This might be due to network connectivity issues - check internet connection',
        );
      }
    }
  }
  ...
  static Future<void> _sendFCMNotificationToAdminOnly(
    ...
    try {
      print(
        '🔍 _sendFCMNotificationToAdminOnly called for adminUserId: $adminUserId',
      );
      print('   Title: $title');
      print('   Body: $body');
      ...
      try {
        final adminDoc =
            await _firestore.collection('admin').doc(adminUserId).get();
        if (adminDoc.exists) {
          final data = adminDoc.data()!;
      ...
      if (fcmTokens.isNotEmpty) {
        ...
        print(
          '📊 Admin FCM Summary: $successCount/$totalTokens successful for admin: $adminUserId',
        );
      } else {
  ...
  static Future<void> sendNewUserRegisteredNotificationToAdmin({
    ...
    try {
      ...
      final teacherCheck =
          await _firestore.collection('teachers').doc(adminUserId).get();
      final studentCheck =
          await _firestore.collection('students').doc(adminUserId).get();

      if (teacherCheck.exists) {
        print(
          '⚠️ WARNING: Admin user also exists in teachers collection! This could cause notification issues.',
        );
      ...
      if (studentCheck.exists) {
        print(
          '⚠️ WARNING: Admin user also exists in students collection! This could cause notification issues.',
        );
      ...
      if (!teacherCheck.exists && !studentCheck.exists) {
        ...
        print(
          '⚠️ Admin user found in multiple collections - proceeding with caution',
        );
      }
      ...
      print(
        '💾 Saving notification to admin collection for adminUserId: $adminUserId',
      );
      ...
      print(
        '✅ Registration notification process completed for admin: $adminUserId',
      );
    } catch (e) {
      if (kDebugMode) {
        print(
          '❌ Error sending new user registration notification to admin: $e',
        );
      }
    }
  }
  ...
  static Future<void> sendUserAssignedNotification({
    ...
    try {
      ...
      print(
        '   Message: "You have been assigned a $assignedRole: $assignedName."',
      );
      ...
      if (assignedRole == 'teacher') {
        ...
        if (!studentDoc.exists) {
          print(
            '❌ SECURITY ERROR: Trying to send teacher assignment to non-student user: $userId',
          );
          print(
            '   This notification says "You have been assigned a teacher" but user is not in students collection',
          );
          return;
        }
        final studentData = studentDoc.data()!;
        print(
          '✅ Verified: Sending teacher assignment notification to student: ${studentData['fullName']}',
        );
        ...
        if (!teacherDoc.exists) {
          print(
            '❌ SECURITY ERROR: Trying to send student assignment to non-teacher user: $userId',
          );
          print(
            '   This notification says "You have been assigned a student" but user is not in teachers collection',
          );
          return;
        }
        final teacherData = teacherDoc.data()!;
        print(
          '✅ Verified: Sending student assignment notification to teacher: ${teacherData['fullName']}',
        );
      }
      ...
      if (kDebugMode) {
        print(
          '✅ Assignment notification sent to user ($userId) with background support',
        );
      }
    } catch (e) {
      if (kDebugMode) {
  ...
  static Future<void> sendTeacherStudentAssignmentNotifications({
    ...
    try {
      print(
        '🔔 NotificationService.sendTeacherStudentAssignmentNotifications called:',
      );
      ...
      if (!teacherDoc.exists) {
        print(
          '❌ ERROR: Teacher ID $teacherId not found in teachers collection!',
        );
        return;
      }
      ...
      if (!studentDoc.exists) {
        print(
          '❌ ERROR: Student ID $studentId not found in students collection!',
        );
        return;
      }
    ...
    for (int attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (!hasConnection) {
          if (kDebugMode) {
            print(
              '⚠️ No network connection - notification will be sent when connection is restored',
            );
          }
        ...
        if (attempt == maxRetries) {
          if (kDebugMode) {
            print(
              '💥 All $maxRetries notification attempts failed for user: $userId',
            );
          }
          rethrow;
        }
  ...
  static Future<void> fixExistingReminders() async {
    try {
      ...
      if (kDebugMode) {
        print(
          '🔍 Found ${pendingReminders.docs.length} pending reminders to check',
        );
      }
      ...
      for (final doc in pendingReminders.docs) {
        ...
        if ((studentId == user.uid || teacherId == user.uid) &&
            (data['studentFcmToken'] == null ||
                data['teacherFcmToken'] == null)) {
          ...
          if (kDebugMode) {
            print(
              '🔧 Fixed reminder for class ${data['classDate']} ${data['classTime']}',
            );
            print(
              '   👨‍🎓 Student token: ${studentFcmToken != null ? 'Added' : 'Not found'}',
            );
            print(
              '   👨‍🏫 Teacher token: ${teacherFcmToken != null ? 'Added' : 'Not found'}',
            );
          }
        }
      }
  ...
  static Future<void> _handleiOSFCMToken() async {
    try {
      ...
      try {
        ...
        if (isSimulator && kDebugMode) {
          print(
            '📱 Running on iOS Simulator - FCM token functionality is limited',
          );
        }
      } catch (e) {
        if (kDebugMode) {
      ...
      final maxTokenRetries =
          isSimulator ? 2 : 5; // Fewer retries for simulator
      ...
      while (token == null && tokenRetryCount < maxTokenRetries) {
        try {
          ...
          if (kDebugMode) {
            print(
              '⚠️ FCM token request failed (attempt ${tokenRetryCount + 1}): $e',
            );
          }
        }
        ...
        if (kDebugMode) {
          print(
            '⏳ FCM token not available yet, retrying... (attempt $tokenRetryCount/$maxTokenRetries)',
          );
        }
      ...
      if (token == null) {
        if (isSimulator) {
          if (kDebugMode) {
            print(
              '📱 FCM token not available on iOS Simulator - this is expected behavior',
            );
          ...
          final mockToken =
              'simulator_token_${DateTime.now().millisecondsSinceEpoch}';
          if (kDebugMode) {
          ...
          if (kDebugMode) {
            print(
              '⚠️ FCM token is still null on physical iOS device after $maxTokenRetries attempts',
            );
            print(
              '🔍 This might indicate APNS configuration issues or network problems',
            );
            print(
              '💡 Check: 1) APNS certificates 2) Bundle ID 3) Network connectivity',
            );
          }
        }
      }
  ...
  static Future<Map<String, dynamic>> validateNotificationSetup() async {
    ...
    try {
      ...
      try {
        ...
        results['details']['permissionStatus'] =
        ...
        try {
          ...
          if (apnsToken == null) {
            results['warnings'].add(
              'APNS token not available (normal on simulator)',
            );
          }
        } catch (e) {
      ...
      if (user != null) {
        ...
        for (final collection in collections) {
          try {
            ...
            if (doc.exists) {
              ...
              if (tokens.isEmpty) {
                results['warnings'].add(
                  'User has no FCM tokens stored in Firestore',
                );
              }
              break;
            }
        ...
        if (!foundUser) {
          results['issues'].add(
            'User not found in any collection (teachers/students/admin)',
          );
          results['overall'] = false;
        }
      } else {

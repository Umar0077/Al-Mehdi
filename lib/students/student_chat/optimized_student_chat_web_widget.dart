...
class OptimizedStudentChatWebWidget extends StatefulWidget {
  final Map<String, dynamic> chat;
  final bool showHeader;

  const OptimizedStudentChatWebWidget({
    super.key,
    required this.chat,
  ...
  @override
  State<OptimizedStudentChatWebWidget> createState() => _OptimizedStudentChatWebWidgetState();
}

class _OptimizedStudentChatWebWidgetState extends State<OptimizedStudentChatWebWidget> {
  ...
  void _sendMessageFast() async {
    ...
    final provider = Provider.of<StudentChatWebProvider>(context, listen: false);
    ...
    if (kDebugMode) {
      print('📤 Student web sending message optimistically: $message');
    }
    ...
    return Align(
      ...
      child: Container(
        ...
        constraints: BoxConstraints(
          maxWidth: MediaQuery.of(context).size.width * 0.6, // Responsive width for web
        ),
        decoration: BoxDecoration(
          color: isMe ? meColor : fromColor,
          borderRadius: BorderRadius.circular(12),
          border: status == 'failed' ? Border.all(color: Colors.red, width: 1) : null,
        ),
        child: Column(
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<StudentChatWebProvider>(
                ...
                child: Row(
                  children: [
                    CircleAvatar(
                      ...
                      radius: 20,
                      child: (widget.chat['avatar'] ?? '').isEmpty ? Icon(Icons.person, color: Colors.white, size: 22) : null,
                    ),
                ...
                builder: (context, snapshot) {
                  ...
                  if (snapshot.hasData && snapshot.data!.docs.isNotEmpty) {
                    ...
                    sortedMessages.sort((a, b) {
                      ...
                      return (aTime?.toDate() ?? DateTime(1970))
                          .compareTo(bTime?.toDate() ?? DateTime(1970));
                    });
                  ...
                  for (var localMessage in provider.localMessages) {
                    ...
                    messageWidgets.add(
                      _buildMessageBubble(
                        ...
                        isMe: true,
                        time: _formatTime(localMessage['timestamp'] as Timestamp?),
                        status: status,
                        tempId: tempId,
                      ),
                    );
                    ...
                    child: TextField(
                      ...
                      decoration: InputDecoration(
                        ...
                        focusedBorder: OutlineInputBorder(
                          ...
                          borderSide: const BorderSide(color: appGreen, width: 2.0),
                        ),

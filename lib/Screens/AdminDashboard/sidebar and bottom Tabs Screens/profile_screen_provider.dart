...
class ProfileScreenProvider extends ChangeNotifier {
  ...
  Future<void> loadAdminProfile() async {
    isLoading = true;
    notifyListeners();
    final adminDoc = await FirebaseFirestore.instance.collection('admin').limit(1).get();
  ...
  Future<void> _uploadImage(Uint8List imageBytes) async {
    try {
      ...
      if (imageBytes.length > 10 * 1024 * 1024) {
        throw Exception('File size too large. Please select an image smaller than 10MB.');
      }
      ...
      if (adminDoc.docs.isNotEmpty) {
        ...
        if (profileImageUrl != null && profileImageUrl!.isNotEmpty) {
          try {
            final previousRef = FirebaseStorage.instance.refFromURL(profileImageUrl!);
        ...
        final ref = FirebaseStorage.instance
            .ref()
            .child('admin_profile_pictures/$fileName');
        ...
        uploadTask.snapshotEvents.listen((TaskSnapshot snapshot) {
          ...
          if (kDebugMode) {
            print('📤 Admin upload progress: ${(progress * 100).toStringAsFixed(1)}%');
          }
        });

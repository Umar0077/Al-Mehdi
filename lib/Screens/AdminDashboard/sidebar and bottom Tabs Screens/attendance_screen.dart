...
import 'attendance_content_provider.dart';
...
class AttendanceContent extends StatelessWidget {
                  ...
                  Container(
                    ...
                    child: Column(
                      children: [
                        Row(
                          children: [
                            Expanded(child: _buildDropdown(context, provider, "Teacher")),
                            const SizedBox(width: 10),
                            Expanded(child: _buildDropdown(context, provider, "Student")),
                          ],
                        ),
  ...
  Widget _buildDropdown(BuildContext context, AttendanceContentProvider provider, String hint) {
    Color dropdownColor =
        Theme.of(context).brightness == Brightness.dark
            ? darkBackground
            : appLightGreen;
    if (hint == "Teacher") {
      List<Map<String, dynamic>> filteredTeachers = List.from(provider.teachers);
      if (provider.selectedStudentId != null) {
        ...
        filteredTeachers =
            provider.teachers.where((t) => t['id'] == assignedTeacherId).toList();
      }
      ...
      List<Map<String, dynamic>> filteredStudents = List.from(provider.students);
      if (provider.selectedTeacherId != null) {
        filteredStudents =
            provider.students
                .where((s) => s['assignedTeacherId'] == provider.selectedTeacherId)
                .toList();
      }
      if (provider.searchQuery.isNotEmpty) {
        filteredStudents =
            ...
            child: DropdownButton<String>(
              ...
              onChanged: (String? newValue) {
                ...
                if (provider.selectedTeacherId != null) {
                  ...
                  if (student['assignedTeacherId'] != provider.selectedTeacherId) {
  ...
  Widget _buildFilterRow(BuildContext context, AttendanceContentProvider provider) {
    return Padding(
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        final classes = snapshot.data!.docs.where((doc) {
          ...
          if (selectedTeacherId != null && data['teacherId'] != selectedTeacherId) {
            return false;
          }
          ...
          if (selectedStudentId != null) {
            ...
            if (studentId != selectedStudentId && studentName != selectedStudentId) {
              return false;
            }
          }
          ...
          itemBuilder: (context, index) {
            ...
            return FutureBuilder<DocumentSnapshot?>(
              future: teacherId.isNotEmpty 
                ? FirebaseFirestore.instance.collection('teachers').doc(teacherId).get()
                : null,
              builder: (context, teacherSnapshot) {
                ...
                String finalStudentName = studentName.isNotEmpty ? studentName : 'Unknown Student';
                
                return FutureBuilder<DocumentSnapshot?>(
                  future: studentId.isNotEmpty && studentName.isEmpty
                    ? FirebaseFirestore.instance.collection('students').doc(studentId).get()
                    : null,
                  builder: (context, studentSnapshot) {
                    if (studentSnapshot.hasData && studentSnapshot.data!.exists) {
                      finalStudentName = studentSnapshot.data!['fullName'] ?? 'Student';
                    }

...
import 'fees_status_provider.dart';
...
class _FeesStatusView extends StatelessWidget {
                      ...
                      decoration: InputDecoration(
                        ...
                        suffixIcon:
                            provider.searchController.text.isNotEmpty
                                ? IconButton(
                                  icon: const Icon(Icons.clear, color: appGreen),
                                  onPressed: () {
                        ...
                        children: [
                          ...
                          InkWell(
                            onTap: () async {
                              ...
                              if (picked != null) {
                                provider.setMonth(DateTime(picked.year, picked.month));
                              }
                            },
                            child: Row(
                              mainAxisSize: MainAxisSize.min,
                                  ...
                                  Expanded(
                                    ...
                                    child: Container(
                                      ...
                                      child: Text(
                                        user['role'] ?? '',
                                        style: TextStyle(
                                          color: provider.getRoleColor(user['role'] ?? ''),
                                          fontWeight: FontWeight.bold,
                                          fontSize: isSmallScreen ? 11 : (isMobile ? 13 : 15),
                                        ),
                                      ),
                                            ...
                                            ? GestureDetector(
                                                onTap: () async {
                                                  final newStatus = await showDialog<String>(
                                                    context: context,
                                                    builder: (context) {
                                                      bool isPaid = user['feeStatus'] == 'Paid';
                                                      return AlertDialog(
                                                        backgroundColor:
                                                            Theme.of(context).scaffoldBackgroundColor,
                                                        title: const Center(
                                                          child: Text('Change Fee Status'),
                                                        ),
                                                  ...
                                                  if (newStatus != null && newStatus != user['feeStatus']) {
                                                    provider.updateFeeStatus(user, newStatus);
                                                  }
                                                },
                                                child: Container(
                                                  padding: const EdgeInsets.symmetric(
                                                  ...
                                                  child: Text(
                                                    ...
                                                    style: TextStyle(
                                                      color: provider.getFeeStatusColor(user['feeStatus'] ?? ''),
                                                      fontWeight: FontWeight.bold,
                                                      fontSize: isSmallScreen ? 11 : (isMobile ? 13 : 15),
                                                    ),
                                                  ),
  ...
  Widget _mobileLayout(BuildContext context, FeesStatusProvider provider, Color dropdownColor, double screenWidth, bool isWeb) {
    return PopScope(
      canPop: false,
      onPopInvoked: (didPop) {
        if (didPop) return;
      ...
      child: Scaffold(
        ...
        body: _feesStatusBody(context, provider, dropdownColor, screenWidth, isWeb),
      ),
    );
  }
}

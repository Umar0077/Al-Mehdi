...
class ScheduleClassProvider extends ChangeNotifier {
  ...
  Future<void> _sendClassReminderNotification(
    ...
    try {
      ...
      print(
        'Sending admin reminder notification for class: $classDate at $classTime',
      );
      ...
      print(
        'Admin reminder sent successfully for class at $classTime on $classDate',
      );
    } catch (e) {
  ...
  List<Map<String, dynamic>> get filteredTeachers {
    ...
    return teachers
        .where((t) => t['uid'] == student['assignedTeacherId'])
        .toList();
  }
  ...
  List<Map<String, dynamic>> get filteredStudents {
    ...
    final teacher = teachers.firstWhere(
      (t) => t['uid'] == selectedTeacherId,
      orElse: () => {},
    );
    if (teacher.isEmpty) return students;
    final assignedStudentIds =
        (teacher['assignedStudentIds'] as List?)?.cast<String>() ?? [];
    return students
        .where(
          (s) =>
              s['assignedTeacherId'] == teacher['uid'] ||
              assignedStudentIds.contains(s['uid']),
        )
        .toList();
  }
  ...
  void setSelectedTeacherId(String? val) {
    ...
    if (selectedStudentId != null) {
      ...
      if (teacher.isNotEmpty &&
          !teacher['assignedStudentIds'].contains(selectedStudentId)) {
  ...
  void setSelectedStudentId(String? val) {
    ...
    if (selectedTeacherId != null) {
      ...
      if (student.isNotEmpty &&
          student['assignedTeacherId'] != selectedTeacherId) {
  ...
  Future<void> _scheduleSingleClass(
    ...
    final classData = {
      ...
      'jitsiRoom':
          '${selectedTeacherId}_${DateTime.now().millisecondsSinceEpoch}',
      'status': 'upcoming',
    ...
    _sendSingleSummaryNotification(
      selectedStudentId!,
      selectedTeacherId!,
      teacher['name'],
      selectedDate!,
  ...
  Future<void> _scheduleRecurringClasses(
    ...
    print(
      'Admin scheduling ${classDates.length} classes for $selectedScheduleType over $selectedDuration',
    );
  ...
  void _sendSingleSummaryNotification(
    String studentId,
    String teacherId,
    String teacherName,
    String classDate,
    ...
    (() async {
      try {
        ...
        if (isRecurring) {
          ...
          notificationMessage =
              'Your classes have been scheduled for $durationText on $scheduleTypeText';
        } else {
          // For single class
          notificationMessage =
              'Your class has been scheduled at $classTime on $classDate';
        }
  ...
  void _scheduleClassReminders(List<Map<String, dynamic>> reminderData) {
    ...
    (() async {
      try {
        print(
          '🔄 Admin starting to schedule ${reminderData.length} class reminders...',
        );
        final batch = FirebaseFirestore.instance.batch();
          ...
          if (reminderTime.isAfter(now)) {
            final reminderDoc = {
              ...
              'notificationData': {
                ...
                'body':
                    'Your class with ${data['teacherName']} starts in 5 minutes. Time: ${data['time']} on ${data['date']}',
                'teacherTitle': '🔔 Class Starting Soon!',
                'teacherBody':
                    'Your class with ${data['studentName']} starts in 5 minutes. Time: ${data['time']} on ${data['date']}',
              },
            };
        ...
        if (scheduledCount > 0) {
          await batch.commit();
          print(
            '✅ Admin successfully scheduled $scheduledCount class reminders with background support',
          );
        } else {
  ...
  void setFilterTeacherId(String? val) {
    ...
    if (filterStudentId != null) {
      ...
      if (teacher.isNotEmpty &&
          !teacher['assignedStudentIds'].contains(filterStudentId)) {
    ...
    filteredScheduledClasses =
        scheduledClasses.where((classData) {
          ...
          if (filterTeacherId != null &&
              classData['teacherId'] != filterTeacherId) {
            return false;
          }

          // Filter by student
          if (filterStudentId != null &&
              classData['studentId'] != filterStudentId) {
            return false;
          }
          ...
          if (filterStartDate != null || filterEndDate != null) {
            try {
              final classDate = DateFormat(
                'MM/dd/yyyy',
              ).parse(classData['date']);

              if (filterStartDate != null) {
                final startDate = DateFormat(
                  'MM/dd/yyyy',
                ).parse(filterStartDate!);
  ...
  Future<void> cancelMultipleClasses(List<String> classIds) async {
    try {
      ...
      filteredScheduledClasses.removeWhere(
        (cls) => classIds.contains(cls['id']),
      );
                ...
                try {
                  ...
                  if (context.mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text(
                          '${classIds.length} classes cancelled successfully',
                        ),
                      ),
                    );
                  }

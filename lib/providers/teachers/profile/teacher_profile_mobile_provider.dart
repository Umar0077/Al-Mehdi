  ...
  Future<void> uploadProfilePicture(BuildContext context) async {
    ...
    try {
      ...
      final result = await uploadService.uploadProfilePicture(
        ...
        onProgress: (progress) {
          if (kDebugMode) {
            print('📤 Teacher mobile upload progress: ${(progress * 100).toStringAsFixed(1)}%');
          }
        },
      );
      ...
      if (result.success && result.downloadUrl != null) {
        ...
        if (kDebugMode) {
          print('✅ Teacher mobile profile picture uploaded successfully');
        }
      } else {
      ...
      if (kDebugMode) {
        print('❌ Error uploading teacher mobile profile picture: $e');
      }

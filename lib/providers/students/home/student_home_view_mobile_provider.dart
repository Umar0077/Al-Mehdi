    ...
    if (joinableClass == null) {
      if (context.mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('No class to join.')));
      }
      return;
    }
    try {
      ...
      // Join Jitsi meeting with comprehensive error handling
      try {
        ...
        // If Jitsi fails, show error but don't crash
        if (context.mounted) {
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(
            const SnackBar(
              content: Text('Unable to join meeting. Please check permissions.'),
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(
          ...
          SnackBar(
            content: Text('Failed to join class: ${e.toString().replaceAll('Exception:', '').trim()}'),

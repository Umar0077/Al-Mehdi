...
class TeacherScheduleClassWebProvider extends ChangeNotifier {
    ...
    try {
      ...
      docs.sort((a, b) {
        ...
        try {
          ...
          if (aDate.isNotEmpty && aTime.isNotEmpty && bDate.isNotEmpty && bTime.isNotEmpty) {
  ...
  void onStudentChanged(String? value) {
    ...
    selectedStudentName = assignedStudents.firstWhere(
      (s) => s['id'] == value,
      orElse: () => {'name': ''},
    )['name'];
    notifyListeners();
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          localDateTime = DateFormat('yyyy-MM-dd hh:mm a').parse('$selectedDate $selectedTime');
        }
      ...
      final bool isRecurring = selectedScheduleType != null && selectedDuration != null;
  ...
  Future<void> _scheduleSingleClass(String teacherId, String teacherName, DateTime localDateTime) async {
  ...
  Future<void> _scheduleRecurringClasses(String teacherId, String teacherName, DateTime startDateTime) async {
    ...
    print('Scheduling ${classDates.length} classes for ${selectedScheduleType} over ${selectedDuration}');
    ...
    (() async {
      try {
        ...
        if (selectedScheduleType != null && selectedDuration != null) {
          ...
          scheduleMessage = 'Your classes have been scheduled for $durationLabel on $scheduleTypeLabel';
        } else {
          scheduleMessage = 'Your class has been scheduled for ${selectedDate!} at ${selectedTime!}';
        }
  ...
  void _scheduleClassReminders(List<Map<String, dynamic>> reminderData) {
    ...
    (() async {
      try {
        ...
        await batch.commit();
        print('Teacher scheduled ${reminderData.length} class reminders with background notification support');
  ...
  Future<void> _sendClassReminderNotification(Map<String, dynamic> reminderData) async {
    try {
      ...
      print('Sending reminder notification for class: $classDate at $classTime');
  ...
  Future<void> showCancelClassDialog(BuildContext context, String classId, String classDate, String classTime) async {

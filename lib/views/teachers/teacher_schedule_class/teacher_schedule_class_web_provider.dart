...
class TeacherScheduleClassWebProvider extends ChangeNotifier {
  ...
  Future<void> loadAssignedStudents() async {
    ...
    try {
      ...
      assignedStudents =
          studentsSnapshot.docs.map((doc) {
            try {
              ...
              // Safely handle fcmTokens
              if (data.containsKey('fcmTokens') && data['fcmTokens'] != null) {
                try {
                  final tokens = List<String>.from(data['fcmTokens'] as List);
                  fcmTokens[studentId] = tokens;
                } catch (e) {
                  print('Error parsing fcmTokens for student $studentId: $e');
                }
              }

              // Safely get student name with fallback
              final String studentName = 
                  (data.containsKey('fullName') && data['fullName'] != null)
              ...
              print('Error processing student ${doc.id}: $e');
              return {
                'id': doc.id,
                'name': 'Student',
              };
      ...
      assignedStudents.removeWhere((student) => 
          student['id'] == null || 
          (student['id'] as String).isEmpty);
      
      print('Loaded ${assignedStudents.length} assigned students');
      notifyListeners();
    } catch (e) {
  ...
  void onStudentChanged(String? value) {
    try {
      ...
      if (value != null && value.isNotEmpty) {
        ...
        final student = assignedStudents.firstWhere(
          (s) => s['id'] == value,
          orElse: () => {'id': '', 'name': ''},
        );
        
        selectedStudentName = 
            (student['name'] != null && (student['name'] as String).isNotEmpty)
      ...
      print('Error in onStudentChanged: $e');
